...
export default function ThemesPage() {
  ...
  return (
    <div className="bg-primary-100 flex flex-col -mt-4 items-center">
      <div className="mt-[2%] pl-5 relative flex flex-row bg-white border border-primary-200 text-black group rounded-lg items-center gap-4 w-11/12 md:w-4/5 lg:w-2/3 xl:w-1/2 focus-within:outline focus-within:outline-blue-500">
        <input
          type="text"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-5 md:gap-8 lg:gap-10 xl:gap-12 mt-20 justify-center w-full sm:w-5/6 px-2">
        {themes.map((theme, index) => (
          <div
            key={index}
            className={`relative border-8 border-primary-100 hover:border-white group`}
            onClick={() => {
          ...
          >
            <Image
              ...
              src={theme.src}
              alt={theme.name}
              className={`object-cover h-full w-full aspect-[16/9] group-hover: transition-transform duration-300 ease-in-out`}
            />
            <div className="absolute inset-x-0 bottom-0 px-3">
              <h1 className="text-paragraph sm:text-heading-06 md:text-heading-05 lg:text-heading-04 xl:text-heading-03 font-bold drop-shadow-md text-white">
                {theme.name}
              </h1>
            </div>
          </div>
